...
uchar Read_MFRC522(uchar addr)
{
	...
	val = RC522_SPI_Transfer(0x00); // tạo clock để nhận data
...
{
	ClearBitMask(TxControlReg, 0x03); // 0x03 Chuẩn ISO 14443 dùng 2 chân để tăng hiệu suất và giảm nhiễu
}
 ...
 * Return value: the successful return MI_OK
 * 
 * // lưu dữ liệu của thẻ vào bộ nhớ đệm
 */
 ...
 * Return value: the successful return MI_OK
 * để đọc mã định danh duy nhất củ 1 thẻ nếu có nhiều thẻ cùng 1 lúc và ghi ra 4 byte của card
 */
...
{
    ClearBitMask(Status2Reg, 0x08);   // Clear MFCrypto1On bit
}